...
/*Implementing Red Black tree using the standard Java BST implementation */
...
public class RedBlackTree extends BinarySearchTree {
    ...
    //Constructing empty red blabk tree
    public RedBlackTree() {
        this(null);
    }

    //Ordering items according to the comparator
    public RedBlackTree(Comparator c) {
        super(c);
    }

    //Adding the field color to the structure of each node
    class Node extends LinkedBinaryTreeNode {
        Color color = Color.black; //Setting default as black
    ...
    //Inserting an item into the tree. If the same element exists, it's overwritten.
    ...
    //Deletes the node containing the given value
    public void remove(Object data) {
        ...
        if (node == null) {
            ...
            // Node has two children, Copy inorder predecessor.
        ...
        if (pullUp != null) {
            ...
            // Nothing to pull up when deleting a root; empty tree
            setRoot(null);
        } else {
    ...
    //add() method takes care of the insertion cases
    //This method handles recoloring and rotation
    private void adjustAfterInsertion(Node n) {
        // Step 1: color the node red
        setColor(n, Color.red);

        // Step 2: Correcting red-red problems, if they exist
        if (n != null && n != root && isRed(parentOf(n))) {

            // Step 2a : Recolor, recursively check higher nodes
    ...
    //remove() method handles the standard delete cases. 
    //This method handles recoloring, rotation and handling double-black
    //nodes
